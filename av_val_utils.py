...
def collate_fn_av(batch):
    ...
    for sample in batch:
        for idx,clip in enumerate(sample['clip']):
            ...
            new_batchs['audio'].append(sample['audio'])
            new_batchs['target'].append(sample['target'][idx])

    
    batchs['audio'] = default_collate(new_batchs['audio'])
    batchs['clip'] = default_collate(new_batchs['clip'])
    batchs['target'] = default_collate(new_batchs['target'])
    batchs["video_name"] = default_collate(new_batchs["video_name"])
    return batchs
...
def get_av_inference_utils(opt,inference_subset):
    ...
    inference_data =  AudioVideoDatasetMultiClips(opt.video_dir,
                                            opt.audio_dir,
                                             opt.annotation_path,
                                             inference_subset,
                                             opt.fps,
